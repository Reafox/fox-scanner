...
namespace fox_scanner{
    using Fox_token = ascaner::Token<fox_scanner::Lexeme_info>;
 
    class Scanner : public ascaner::Abstract_scaner<fox_scanner::Lexeme_info>{
    public:
        Scanner()               = default;
        Scanner(const ascaner::Location_ptr& location, const Errors_and_tries& et) :
            ascaner::Abstract_scaner<fox_scanner::Lexeme_info>(location, et) {};
        Scanner(const Scanner&) = default;
        virtual ~Scanner()      = default;

        ascaner::Token<fox_scanner::Lexeme_info> current_lexeme() override;

        std::string lexeme_to_string(const fox_scanner::Lexeme_info& li) override;
    private:
